  ...
  return (
    <MainLayout>
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-verde">Clube</h1>
          <p className="text-gray-500 mt-1">Planos, membros e acompanhamento de procedimentos</p>
        </div>
        <div className="flex gap-3">
          {tab === "planos" && (
            <button onClick={() => setShowPlanModal(true)} className="bg-verde hover:bg-verde-900 text-white px-4 py-2.5 rounded-xl flex items-center gap-2 text-sm transition">
              <Plus size={16} /> Novo plano
            </button>
          )}
          {tab === "membros" && (
            <button onClick={() => setShowMemberModal(true)} className="bg-verde hover:bg-verde-900 text-white px-4 py-2.5 rounded-xl flex items-center gap-2 text-sm transition">
      ...
      {/* TABS */}
      <div className="flex gap-1 mb-6 bg-creme-100 p-1 rounded-xl w-fit">
        {[
          ...
          <button
            key={key}
            onClick={() => setTab(key)}
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition ${tab === key ? "bg-white text-verde shadow-sm" : "text-gray-500 hover:text-verde"}`}
          >
            <Icon size={15} />
            {label}
          </button>
      ...
      {loading ? <Spinner /> : (
        <>
          ...
          {tab === "planos" && (
            plans.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-24 text-center">
                <Star size={48} className="text-ambar mb-4" />
                <h2 className="text-xl font-semibold text-verde mb-2">Nenhum plano cadastrado</h2>
                <p className="text-gray-500 mb-6">Crie os planos do clube com os procedimentos incluídos.</p>
                <button onClick={() => setShowPlanModal(true)} className="bg-verde hover:bg-verde-900 text-white px-5 py-3 rounded-xl flex items-center gap-2 transition">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                {plans.map((plan) => (
                  <div key={plan.id} className="bg-creme-50 border border-creme-200 rounded-2xl p-5">
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <h2 className="text-lg font-bold text-verde">{plan.name}</h2>
                        {plan.description && <p className="text-sm text-gray-500 mt-0.5">{plan.description}</p>}
                      </div>
                      <div className="flex gap-2">
                        <button onClick={() => openEditPlan(plan)} className="text-verde hover:text-verde-900"><Pencil size={15} /></button>
                    ...
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        <p className="text-2xl font-bold text-verde">{fmt(plan.price)}</p>
                        <p className="text-xs text-gray-500">{plan.billingCycle}</p>
                      </div>
                      <div className="bg-verde text-white text-xs px-2.5 py-1 rounded-full">
                    ...
                    <div className="space-y-1.5">
                      {plan.items.map((item) => (
                        <div key={item.id} className="flex items-center justify-between bg-white border border-creme-200 rounded-lg px-3 py-2 text-sm">
                          <span className="font-medium text-verde">{item.quantity}x {item.procedureName}</span>
          ...
          {tab === "membros" && (
            members.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-24 text-center">
                <Users size={48} className="text-ambar mb-4" />
                <h2 className="text-xl font-semibold text-verde mb-2">Nenhum membro cadastrado</h2>
                <p className="text-gray-500 mb-6">Associe pacientes aos planos do clube.</p>
                <button onClick={() => setShowMemberModal(true)} className="bg-verde hover:bg-verde-900 text-white px-5 py-3 rounded-xl flex items-center gap-2 transition">
              ...
              <div className="bg-creme-50 border border-creme-200 rounded-2xl overflow-hidden">
                <table className="w-full">
                  <thead className="bg-creme-100">
                    <tr>
                      <th className="text-left p-4 text-verde text-sm font-semibold">Paciente</th>
                      <th className="text-left p-4 text-verde text-sm font-semibold">Plano</th>
                      <th className="text-left p-4 text-verde text-sm font-semibold">Adesão</th>
                      <th className="text-left p-4 text-verde text-sm font-semibold">Validade</th>
                      <th className="text-left p-4 text-verde text-sm font-semibold">Pagamento</th>
                      <th className="text-left p-4 text-verde text-sm font-semibold">Status</th>
                      <th className="p-4" />
                    </tr>
                  </thead>
                  <tbody>
                    {members.map((m) => (
                      <tr key={m.id} className="border-t border-creme-200 hover:bg-[#F3EEE5] transition">
                        <td className="p-4">
                          <p className="font-medium text-verde text-sm">{m.patient.name}</p>
                          <p className="text-xs text-gray-400">{m.patient.phone}</p>
                        </td>
                        <td className="p-4 text-sm text-verde">{m.plan.name}</td>
          ...
          {tab === "alertas" && (
            alerts.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-24 text-center">
                <CheckCircle size={48} className="text-verde mb-4" />
                <h2 className="text-xl font-semibold text-verde mb-2">Tudo em dia!</h2>
                ...
                {alerts.map((alert, i) => (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="font-semibold text-verde text-sm">{alert.patientName}</p>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <button
                        ...
                        className="bg-verde hover:bg-verde-900 text-white text-xs px-3 py-1.5 rounded-lg transition"
                      >
                        Registrar
                      </button>
                    </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-6">
          <div className="bg-white rounded-2xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-verde">{editingPlan ? "Editar plano" : "Novo plano"}</h2>
              <button onClick={closePlanModal}><X size={20} /></button>
            </div>
            <form onSubmit={savePlan} className="space-y-4">
              <input value={planName} onChange={e => setPlanName(e.target.value)} placeholder="Nome do plano" required className="w-full border border-ambar rounded-xl p-3 text-sm" />
              <textarea value={planDesc} onChange={e => setPlanDesc(e.target.value)} placeholder="Descrição (opcional)" rows={2} className="w-full border border-ambar rounded-xl p-3 text-sm" />
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs text-gray-500 block mb-1">Preço (R$)</label>
                  <input value={planPrice} onChange={e => setPlanPrice(e.target.value)} type="number" min="0" step="0.01" placeholder="0,00" required className="w-full border border-ambar rounded-xl p-3 text-sm" />
                </div>
                <div>
                  <label className="text-xs text-gray-500 block mb-1">Ciclo de cobrança</label>
                  <select value={planCycle} onChange={e => setPlanCycle(e.target.value)} className="w-full border border-ambar rounded-xl p-3 text-sm">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <p className="text-sm font-semibold text-verde">Procedimentos incluídos</p>
                  <button type="button" onClick={() => setPlanItems([...planItems, { procedureName: "", quantity: 1, intervalMonths: 12 }])} className="text-xs text-verde border border-ambar px-2.5 py-1 rounded-lg hover:bg-creme-100">
                    + Adicionar
                  </button>
                </div>
                <div className="space-y-2">
                  {planItems.map((item, i) => (
                    <div key={i} className="grid grid-cols-12 gap-2 items-center">
                      <input value={item.procedureName} onChange={e => updatePlanItem(i, "procedureName", e.target.value)} placeholder="Ex: Toxina Botulínica" className="col-span-5 border border-ambar rounded-lg p-2 text-sm" />
                      <div className="col-span-2 relative">
                        <input value={item.quantity} onChange={e => updatePlanItem(i, "quantity", e.target.value)} type="number" min="1" placeholder="Qtd" className="w-full border border-ambar rounded-lg p-2 text-sm" />
                      </div>
                      <div className="col-span-4">
                        <select value={item.intervalMonths} onChange={e => updatePlanItem(i, "intervalMonths", Number(e.target.value))} className="w-full border border-ambar rounded-lg p-2 text-sm">
              ...
              <div className="flex justify-end gap-3 pt-2">
                <button type="button" onClick={closePlanModal} className="border border-ambar px-4 py-2 rounded-xl text-sm">Cancelar</button>
                <button type="submit" className="bg-verde hover:bg-verde-900 text-white px-5 py-2 rounded-xl text-sm">{editingPlan ? "Salvar" : "Criar plano"}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {showMemberModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-6">
          <div className="bg-white rounded-2xl w-full max-w-md p-6">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-verde">Novo membro</h2>
            ...
            <form onSubmit={saveMember} className="space-y-4">
              <div className="relative">
                ...
                <input
                  ...
                  autoComplete="off"
                  className="w-full border border-ambar rounded-xl p-3 text-sm focus:outline-none focus:border-verde"
                />
              ...
              <div>
                ...
                <select value={memberPlanId} onChange={e => setMemberPlanId(e.target.value)} required className="w-full border border-ambar rounded-xl p-3 text-sm">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="date" value={memberStartDate} onChange={e => setMemberStartDate(e.target.value)} required className="w-full border border-ambar rounded-xl p-3 text-sm" />
                </div>
                <div>
                  <label className="text-xs text-gray-500 block mb-1">Validade (opcional)</label>
                  <input type="date" value={memberEndDate} onChange={e => setMemberEndDate(e.target.value)} className="w-full border border-ambar rounded-xl p-3 text-sm" />
                </div>
              </div>
              <div>
                <label className="text-xs text-gray-500 block mb-1">Forma de pagamento</label>
                <select value={memberPayment} onChange={e => setMemberPayment(e.target.value)} className="w-full border border-ambar rounded-xl p-3 text-sm">
                  <option value="">Selecione</option>
                  {PAYMENT_METHODS.map(m => <option key={m} value={m}>{m}</option>)}
                </select>
              </div>
              <textarea value={memberNotes} onChange={e => setMemberNotes(e.target.value)} placeholder="Observações (opcional)" rows={2} className="w-full border border-ambar rounded-xl p-3 text-sm" />
              <div className="flex justify-end gap-3 pt-2">
                <button type="button" onClick={closeMemberModal} className="border border-ambar px-4 py-2 rounded-xl text-sm">Cancelar</button>
                <button type="submit" className="bg-verde hover:bg-verde-900 text-white px-5 py-2 rounded-xl text-sm">Adicionar</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {showAppModal && appAlert && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-60 p-6">
          <div className="bg-white rounded-2xl w-full max-w-sm p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-bold text-verde">Registrar aplicação</h2>
              <button onClick={() => setShowAppModal(false)}><X size={20} /></button>
            </div>
            <p className="text-sm text-gray-500 mb-4">
              <span className="font-medium text-verde">{appAlert.patientName}</span> — {appAlert.quantity}x {appAlert.procedureName}
            </p>
            <form onSubmit={saveApplication} className="space-y-3">
              <div>
                ...
                <input type="date" value={appDate} onChange={e => setAppDate(e.target.value)} required className="w-full border border-ambar rounded-xl p-3 text-sm" />
              </div>
              <textarea value={appNotes} onChange={e => setAppNotes(e.target.value)} placeholder="Observações (opcional)" rows={2} className="w-full border border-ambar rounded-xl p-3 text-sm" />
              <div className="flex justify-end gap-3 pt-1">
                <button type="button" onClick={() => setShowAppModal(false)} className="border border-ambar px-4 py-2 rounded-xl text-sm">Cancelar</button>
                <button type="submit" className="bg-verde hover:bg-verde-900 text-white px-5 py-2 rounded-xl text-sm">Confirmar</button>
              </div>
            </form>
          </div>
        </div>
