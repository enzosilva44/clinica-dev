...
const INPUT = "w-full border border-[#E5D8C5] bg-white rounded-xl px-4 py-2.5 text-sm text-gray-800 placeholder-gray-300 focus:outline-none focus:ring-2 focus:ring-[#00704A]/20 focus:border-[#00704A] transition";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
        <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-[#EFE7DA]">
          <div>
          ...
          <button onClick={onClose} className="w-8 h-8 rounded-xl hover:bg-[#FAF7F2] flex items-center justify-center text-gray-400 transition">
            <X size={16} />
          </button>
        </div>
        <div className="px-6 py-5 space-y-5">
          ...
          {charge.method === "pix" && charge.status === "pending" && (
            <div className="border border-[#E5D8C5] rounded-2xl p-4">
            ...
            <div className="border border-[#E5D8C5] rounded-2xl p-4">
              ...
              <div className="flex items-center gap-2 bg-[#FAF8F5] rounded-xl px-3 py-2.5">
                ...
                <button onClick={() => copy(charge.paymentLink)}
                  className="shrink-0 w-7 h-7 rounded-lg bg-[#00704A] flex items-center justify-center text-white transition hover:bg-[#0A3326]">
          ...
          {(charge.status === "pending" || charge.status === "PENDING" || charge.status === "OVERDUE" || charge.status === "overdue") && (
            <div className="flex flex-col gap-2">
              <div className="flex gap-2">
                <button onClick={() => { copy(charge.paymentLink ?? ""); }}
                  className="flex-1 flex items-center justify-center gap-2 bg-[#00704A] hover:bg-[#0A3326] text-white py-2.5 rounded-xl text-xs font-semibold transition">
                  <Copy size={13} /> Copiar link
                </button>
                <button
                  onClick={handleSendLink}
                  disabled={sendingLink}
                  className="flex-1 flex items-center justify-center gap-2 border border-[#C4895A] text-[#00704A] hover:bg-[#EFE7DA] py-2.5 rounded-xl text-xs font-semibold transition disabled:opacity-60">
                  {sendingLink
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
            ...
            {form.patientId ? (
              ...
              <>
                ...
                {patientResults.length > 0 && (
                  <div className="absolute z-10 top-full mt-1 w-full bg-white border border-[#E5D8C5] rounded-xl shadow-lg overflow-hidden">
                    {patientResults.map((p) => (
                      <button key={p.id} type="button"
                        onClick={() => { set("patientId", p.id); set("patient", p.name); setPatientSearch(""); setPatientResults([]); }}
                        className="w-full text-left px-4 py-2.5 text-sm hover:bg-[#FAF7F2] transition border-b border-[#F0EAE0] last:border-0">
            ...
            <div className="grid grid-cols-3 gap-2">
              {[{ key: "pix", icon: Smartphone, label: "PIX" }, { key: "credit_card", icon: CreditCard, label: "Cartão" }, { key: "boleto", icon: Barcode, label: "Boleto" }].map(({ key, icon: Icon, label }) => (
                <button key={key} type="button" onClick={() => set("method", key)}
                  className={`flex flex-col items-center gap-1.5 py-3 rounded-xl border text-xs font-semibold transition ${form.method === key ? "bg-[#00704A] text-white border-[#00704A]" : "border-[#E5D8C5] text-gray-500 hover:bg-[#FAF7F2]"}`}>
                  <Icon size={16} />{label}
                </button>
              ))}
            </div>
          ...
          {form.amount > 0 && (
            <div className="bg-[#FAF8F5] border border-[#E5D8C5] rounded-xl p-3 text-xs text-gray-500 flex items-center justify-between">
              <span>Total a cobrar</span>
              <span className="font-bold text-[#00704A] text-sm">{fmt(Number(form.amount))}</span>
            </div>
          )}
          <div className="flex gap-2 pt-1">
            <button type="button" onClick={onClose} className="flex-1 py-2.5 rounded-xl border border-[#E5D8C5] text-sm font-medium text-gray-500 hover:bg-[#FAF7F2] transition">Cancelar</button>
            <button type="submit" disabled={loading} className="flex-1 py-2.5 rounded-xl bg-[#00704A] hover:bg-[#0A3326] text-white text-sm font-semibold transition disabled:opacity-60 flex items-center justify-center gap-2">
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white border border-[#E5D8C5] rounded-2xl p-5">
          ...
          {loadingBalance
            ? <div className="h-9 w-28 bg-[#EFE7DA] rounded-lg animate-pulse" />
        ...
        <div className="bg-white border border-[#E5D8C5] rounded-2xl p-5">
          ...
          {loadingBalance
            ? <div className="h-9 w-24 bg-[#EFE7DA] rounded-lg animate-pulse" />
        ...
        <div className="bg-white border border-[#E5D8C5] rounded-2xl p-5">
          ...
          <form onSubmit={handleWithdraw} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <button
                  ...
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] font-bold text-[#00704A] bg-[#EFE7DA] px-2 py-1 rounded-lg hover:bg-[#E5D8C5] transition"
                >
                  Tudo disponível
                </button>
              </div>
            ...
            <button
              ...
              className="w-full py-3 rounded-xl bg-[#00704A] hover:bg-[#0A3326] text-white text-sm font-semibold transition disabled:opacity-50 flex items-center justify-center gap-2"
            >
              {loading
                ...
                return (
                  <div key={w.id} className="flex items-center gap-3 p-3 rounded-xl hover:bg-[#FAF8F5] transition">
                    <div className="w-8 h-8 rounded-xl bg-[#E8F5EE] flex items-center justify-center shrink-0">
                      <ArrowUpFromLine size={13} className="text-[#3A9B6F]" />
                    </div>
  ...
  return (
    <div className="space-y-5">
      ...
      {!apiKey && (
        <div className="bg-[#00704A] rounded-2xl p-5 flex items-center gap-4">
          <div className="w-10 h-10 rounded-xl bg-white/10 flex items-center justify-center shrink-0">
            <Sparkles size={18} className="text-[#C4895A]" />
          </div>
          ...
          <button
            ...
            className="shrink-0 text-xs font-semibold bg-[#C4895A] text-white px-3.5 py-2 rounded-xl flex items-center gap-1.5 hover:bg-[#A88A58] transition"
          >
      ...
      <Section icon={CreditCard} title="Formas de pagamento aceitas">
        <div className="space-y-3">
          {[
            ...
            <div key={key} className="flex items-center justify-between p-3 rounded-xl border border-[#EFE7DA] hover:bg-[#FAF8F5] transition">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-lg bg-[#FAF7F2] flex items-center justify-center">
                  <Icon size={15} className="text-[#00704A]" />
                </div>
                <div>
      ...
      <button
        onClick={save}
        disabled={saving}
        className="w-full py-3 rounded-xl bg-[#00704A] hover:bg-[#0A3326] text-white text-sm font-semibold transition disabled:opacity-60 flex items-center justify-center gap-2"
      >
    ...
    <MainLayout>
        ...
        {tab === "cobrancas" && (
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-2 border border-[#C4895A] text-[#00704A] hover:bg-[#EFE7DA] px-3.5 py-2.5 rounded-xl text-sm font-medium transition"
            >
              <Download size={14} /> Exportar
            </button>
            <button
              onClick={() => setShowNew(true)}
              className="flex items-center gap-2 bg-[#00704A] hover:bg-[#0A3326] text-white px-4 py-2.5 rounded-xl text-sm font-semibold transition"
            >
      ...
      {tab === "cobrancas" && (
        <>
          ...
          {/* filtros */}
          <div className="bg-white border border-[#E5D8C5] rounded-2xl px-4 py-3 mb-5 flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-2 flex-1 min-w-48 border border-[#E5D8C5] rounded-xl px-3 py-2">
            ...
            <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)}
              className="border border-[#E5D8C5] rounded-xl px-3 py-2 text-sm text-gray-600 focus:outline-none bg-white">
          ...
          <div className="bg-white border border-[#E5D8C5] rounded-2xl overflow-hidden">
            <div className="hidden md:grid grid-cols-[2fr_1fr_1fr_1fr_1fr_auto] gap-4 px-5 py-3 border-b border-[#EFE7DA] bg-[#FAF8F5]">
            ...
            {filtered.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 gap-3">
                <Receipt size={32} className="text-[#C4895A] opacity-40" />
                  ...
                  <div key={charge.id}
                    ...
                    <div className="flex items-center gap-1">
                      <button onClick={() => setDetail(charge)}
                        className="w-8 h-8 rounded-xl border border-[#E5D8C5] hover:bg-[#FAF7F2] flex items-center justify-center text-gray-400 hover:text-[#00704A] transition">
                        <Eye size={13} />
                      </button>
                      {(charge.status === "PENDING" || charge.status === "pending" || charge.status === "OVERDUE" || charge.status === "overdue") && (
                        <button
