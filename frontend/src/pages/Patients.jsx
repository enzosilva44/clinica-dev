  ...
  return (
    <MainLayout>
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-[#1F4D46]">Pacientes</h1>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-2 border border-[#C2A56B] hover:bg-[#E8E0D2] disabled:opacity-50 text-[#1F4D46] px-4 py-2.5 rounded-xl transition text-sm font-medium"
          >
      ...
      {showSuggestions && (
        <div className="bg-white border border-[#D8CDB9] rounded-2xl p-5 mb-6">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <Sparkles size={15} className="text-[#1F4D46]" />
              <span className="text-sm font-bold text-[#1F4D46]">Pacientes para reativar</span>
              {suggestions.length > 0 && (
                <span className="text-xs bg-[#E8E0D2] text-[#1F4D46] px-2 py-0.5 rounded-full font-medium">
          ...
          {loadingSuggestions ? (
            <div className="space-y-2">
              {[1, 2, 3].map((i) => (
                <div key={i} className="h-12 bg-[#F5F1EA] rounded-xl animate-pulse" />
              ))}
            </div>
          ) : suggestions.length === 0 ? (
            ...
            <div className="space-y-2">
              {suggestions.map((s, i) => (
                <div key={i} className="flex items-center justify-between bg-[#F5F1EA] border border-[#D8CDB9] rounded-xl px-4 py-3">
                  <div>
                    <p className="text-sm font-semibold text-[#1F4D46]">{s.name}</p>
                  ...
                  <button
                    ...
                    className="text-xs text-[#1F4D46] hover:opacity-70 transition shrink-0 ml-4 font-medium"
                  >
                    Ver paciente →
                  </button>
                </div>
      ...
      <div className="flex flex-col md:flex-row gap-3 mb-6">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 border border-[#C2A56B] rounded-xl text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#1F4D46]/20"
          />
        </div>
        <div className="flex gap-1 bg-[#F5F1EA] border border-[#D8CDB9] rounded-xl p-1">
          {[["active", "Ativos"], ["removed", "Inativos"]].map(([v, l]) => (
            <button
              ...
              className={`px-4 py-1.5 rounded-lg text-sm transition font-medium ${
                status === v ? "bg-[#1F4D46] text-white" : "text-[#1F4D46] hover:bg-[#E8E0D2]"
              }`}
            >
              {l}
            </button>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-28 text-center">
          <div className="w-16 h-16 bg-[#E8E0D2] rounded-2xl flex items-center justify-center mb-4">
            <Users size={28} className="text-[#C2A56B]" />
          </div>
          <h2 className="text-xl font-semibold text-[#1F4D46] mb-2">Nenhum paciente cadastrado</h2>
        ...
        <>
          <div className="bg-[#F5F1EA] border border-[#D8CDB9] rounded-2xl overflow-hidden shadow-sm">
            {/* Cabeçalho da tabela */}
            <div className="px-5 py-3.5 border-b border-[#D8CDB9] bg-[#E8E0D2] flex items-center justify-between">
              <span className="text-sm font-semibold text-[#1F4D46]">
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-[#D8CDB9]">
                  ...
                  <th className="text-left px-5 py-3 text-[#1F4D46] text-xs font-semibold uppercase tracking-wide">Telefone</th>
                  <th className="text-left px-5 py-3 text-[#1F4D46] text-xs font-semibold uppercase tracking-wide hidden md:table-cell">Idade</th>
                  <th className="text-left px-5 py-3 text-[#1F4D46] text-xs font-semibold uppercase tracking-wide hidden md:table-cell">Último agend.</th>
              ...
              <tbody>
                {patients.map((patient) => (
                  <tr
                    ...
                    className="border-t border-[#D8CDB9] cursor-pointer hover:bg-[#F3EEE5] transition group"
                  >
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-full bg-[#1F4D46] text-white text-sm font-semibold flex items-center justify-center shrink-0 select-none">
                          {initials(patient.name)}
                        </div>
                        <div>
                          <p className="text-sm font-semibold text-[#1F4D46] leading-tight">{patient.name}</p>
                          {patient.email && (
                ...
                {patients.length === 0 && (
                  <tr>
                    <td colSpan="5" className="px-5 py-14 text-center text-gray-400 text-sm">
                      Nenhum resultado para <span className="font-medium text-[#1F4D46]">"{search}"</span>
                    </td>
                  </tr>
                )}
              </tbody>
                ...
                {Array.from({ length: Math.min(totalPages, 7) }, (_, i) => i + 1).map((p) => (
                  <button
                    ...
                    className={`w-9 h-9 rounded-xl text-sm transition font-medium ${
                      p === page ? "bg-[#1F4D46] text-white" : "border border-[#C2A56B] hover:bg-[#E8E0D2] text-[#1F4D46]"
                    }`}
                  >
                    {p}
                  </button>
