        ...
        <div key={s.id} className="flex items-center">
          <div className={`flex flex-col items-center gap-1 ${current === s.id ? "opacity-100" : current > s.id ? "opacity-70" : "opacity-30"}`}>
            <div className={`w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold transition
              ${current > s.id ? "bg-[#1F4D46] text-white" : current === s.id ? "bg-[#1F4D46] text-white ring-4 ring-[#1F4D46]/20" : "bg-[#E8E0D2] text-gray-500"}`}>
              {current > s.id ? <Check size={14} /> : s.id}
            </div>
            <p className="text-[10px] text-gray-500 hidden sm:block">{s.label}</p>
          </div>
          {i < STEPS.length - 1 && (
            <div className={`w-8 sm:w-12 h-0.5 mx-1 transition ${current > s.id ? "bg-[#1F4D46]" : "bg-[#E8E0D2]"}`} />
          )}
        </div>
      ))}
    </div>
  ...
  return (
    <div className={`relative w-full h-44 perspective-1000 cursor-pointer select-none`} style={{ perspective: 1000 }}>
      <div className={`relative w-full h-full transition-transform duration-500`}
        ...
        {/* Front */}
        <div className="absolute inset-0 rounded-2xl bg-gradient-to-br from-[#1F4D46] to-[#2D6B60] p-6 flex flex-col justify-between shadow-xl"
          style={{ backfaceVisibility: "hidden" }}>
          <div className="flex justify-between items-start">
            <p className="text-white/70 text-xs font-semibold tracking-widest uppercase">IasoClin Pay</p>
            <div className="flex gap-1">
              <div className="w-8 h-8 rounded-full bg-[#C2A56B] opacity-80" />
              <div className="w-8 h-8 rounded-full bg-[#C2A56B] opacity-50 -ml-4" />
            </div>
          </div>
          <div>
        ...
        {/* Back */}
        <div className="absolute inset-0 rounded-2xl bg-gradient-to-br from-[#285A50] to-[#1F4D46] flex flex-col justify-center shadow-xl"
...
export default function Signup() {
  ...
  return (
    <div className="min-h-screen bg-[#F5F1EA] flex flex-col items-center justify-start px-4 py-10">
      {/* Logo */}
      ...
      <div className="w-full max-w-2xl">
        ...
        {step === 1 && (
          <div>
            <h2 className="text-2xl font-bold text-[#1F4D46] mb-1 text-center">Escolha seu plano</h2>
              ...
              {PLANS.map((p) => (
                <button
                  ...
                  className={`relative text-left rounded-3xl p-6 border-2 transition ${
                    plan === p.id
                      ? "border-[#1F4D46] bg-[#1F4D46] text-white shadow-xl scale-[1.02]"
                      : "border-[#D8CDB9] bg-white hover:border-[#1F4D46]/40"
                  }`}
                >
                  {p.highlight && (
                    <span className={`absolute -top-3 left-1/2 -translate-x-1/2 text-[10px] font-bold px-3 py-1 rounded-full ${plan === p.id ? "bg-[#C2A56B] text-white" : "bg-[#1F4D46] text-white"}`}>
                      MAIS POPULAR
                    </span>
                  )}
                  {plan === p.id && (
                    <div className="absolute top-4 right-4 w-6 h-6 rounded-full bg-[#C2A56B] flex items-center justify-center">
                      <Check size={12} className="text-white" />
                    </div>
                  )}
                  <p className={`text-base font-bold mb-0.5 ${plan === p.id ? "text-white" : "text-[#1F4D46]"}`}>{p.name}</p>
                  <p className={`text-xs mb-4 ${plan === p.id ? "text-white/70" : "text-gray-400"}`}>{p.desc}</p>
                  <p className={`text-3xl font-black mb-1 ${plan === p.id ? "text-white" : "text-[#1F4D46]"}`}>{p.price}</p>
                  ...
                  <ul className="space-y-2">
                    {p.features.map((f) => (
                      <li key={f} className={`flex items-start gap-2 text-xs ${plan === p.id ? "text-white/80" : "text-gray-500"}`}>
                        <Check size={12} className={`mt-0.5 shrink-0 ${plan === p.id ? "text-[#C2A56B]" : "text-[#1F4D46]"}`} />
                        {f}
                      </li>
                    ))}
                  </ul>
            ...
            <button onClick={next}
              className="w-full bg-[#1F4D46] hover:bg-[#285A50] text-white py-4 rounded-2xl font-bold text-sm transition flex items-center justify-center gap-2">
        ...
        {step === 2 && (
          <div className="bg-white rounded-3xl border border-[#E8E0D2] p-8 shadow-sm">
            <h2 className="text-xl font-bold text-[#1F4D46] mb-1">Dados pessoais e da clínica</h2>
                  ...
                  {[{ value: "pf", label: "Pessoa Física", sub: "CPF" }, { value: "pj", label: "Pessoa Jurídica", sub: "CNPJ" }].map((opt) => (
                    <button key={opt.value} type="button" onClick={() => setPersonType(opt.value)}
                      className={`flex-1 py-3 rounded-xl border-2 text-sm font-semibold transition ${
                        personType === opt.value ? "border-[#1F4D46] bg-[#1F4D46] text-white" : "border-[#D8CDB9] text-gray-600 hover:border-[#1F4D46]/40"}`}>
                      {opt.label}
                  ...
                  {[{ value: "F", label: "Dra.", desc: "Feminino" }, { value: "M", label: "Dr.", desc: "Masculino" }].map((opt) => (
                    <button key={opt.value} type="button" onClick={() => setGender(opt.value)}
                      className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl border-2 text-sm font-semibold transition ${
                        gender === opt.value ? "border-[#1F4D46] bg-[#1F4D46] text-white" : "border-[#D8CDB9] text-gray-600 hover:border-[#1F4D46]/40"}`}>
            ...
            <div className="flex gap-3 mt-8">
              <button onClick={back}
                className="flex-1 border border-[#D8CDB9] py-3 rounded-xl text-sm hover:bg-[#F5F1EA] transition flex items-center justify-center gap-2">
                <ChevronLeft size={16} /> Voltar
              </button>
              <button onClick={next}
                className="flex-1 bg-[#1F4D46] hover:bg-[#285A50] text-white py-3 rounded-xl font-semibold text-sm transition flex items-center justify-center gap-2">
        ...
        {step === 3 && (
          <div className="bg-white rounded-3xl border border-[#E8E0D2] p-8 shadow-sm">
            <h2 className="text-xl font-bold text-[#1F4D46] mb-1">Endereço</h2>
        ...
        {step === 4 && (
          <div className="bg-white rounded-3xl border border-[#E8E0D2] p-8 shadow-sm">
            <h2 className="text-xl font-bold text-[#1F4D46] mb-1">Dados de pagamento</h2>
            ...
            <p className="text-[11px] text-gray-400 mt-4 flex items-center gap-1.5">
              <Shield size={12} className="text-[#1F4D46]" />
        ...
        {step === 5 && (
          <div className="bg-white rounded-3xl border border-[#E8E0D2] p-8 shadow-sm">
            <h2 className="text-xl font-bold text-[#1F4D46] mb-1">Criar acesso</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {import.meta.env.VITE_GOOGLE_CLIENT_ID && (
                <>
                  <div className="flex items-center gap-3 my-2">
                    <div className="h-px bg-[#E8E0D2] flex-1" />
              ...
              <div className="flex gap-3 mt-2">
                ...
                <button type="submit" disabled={loading}
                  className="flex-1 bg-[#1F4D46] hover:bg-[#285A50] disabled:opacity-60 text-white py-3 rounded-xl font-semibold text-sm transition flex items-center justify-center gap-2">
        ...
        <p className="text-center text-sm text-gray-400 mt-6">
          Já tem conta?{" "}
          <Link to="/" className="text-[#1F4D46] font-semibold hover:underline">Entrar</Link>
        </p>
      </div>
    </div>
  );
