  ...
  return (
    <MainLayout>
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-[#1F4D46]">Procedimentos</h1>
        ...
        <button
          ...
          className="bg-[#1F4D46] hover:bg-[#285A50] text-white px-4 py-3 rounded-xl flex items-center gap-2 transition"
        >
          <Plus size={18} />
          Novo procedimento
        </button>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 text-center">
          <Stethoscope size={48} className="text-[#C2A56B] mb-4" />
          <h2 className="text-xl font-semibold text-[#1F4D46] mb-2">Nenhum procedimento cadastrado</h2>
          <p className="text-gray-500 mb-6">Cadastre os procedimentos realizados na clínica.</p>
          <button
            onClick={() => { resetForm(); setShowModal(true); }}
            className="bg-[#1F4D46] hover:bg-[#285A50] text-white px-5 py-3 rounded-xl flex items-center gap-2 transition"
          >
            <Plus size={18} />
            Novo procedimento
          </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {procedures.map((procedure) => (
            <div key={procedure.id} className="bg-[#F5F1EA] border border-[#D8CDB9] rounded-2xl p-5">
              {/* TOP */}
              <div className="flex items-start justify-between">
                <div>
                  <h2 className="text-lg font-bold text-[#1F4D46]">{procedure.name}</h2>
                  <p className="text-sm text-gray-500 mt-1">{procedure.category}</p>
                </div>
                <div className="flex gap-3">
                  <button onClick={() => handleEdit(procedure)} className="text-[#1F4D46] hover:text-[#285A50]">
                    <Pencil size={16} />
                  </button>
                  <button onClick={() => handleDuplicate(procedure)} className="text-gray-400 hover:text-[#1F4D46]">
                    <Copy size={16} />
                  </button>
                  <button
              ...
              {/* PRODUTOS */}
              <div className="mt-5">
                <h3 className="font-semibold text-[#1F4D46] mb-2">Produtos:</h3>
                <div className="space-y-2">
                  {!procedure.products?.length && (
                    <p className="text-sm text-gray-400">Nenhum produto vinculado</p>
                  )}
                  {procedure.products?.map((item) => (
                    <div key={item.id} className="bg-white border border-[#D8CDB9] rounded-lg p-3 text-sm">
                      <div className="flex justify-between">
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-6">
          <div className="bg-white rounded-2xl w-full max-w-3xl p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-bold text-[#1F4D46]">
            ...
            <div className="space-y-5">
              <input
                ...
                className="w-full border border-[#C2A56B] rounded-xl p-3"
              />
              <textarea
                value={description}
              ...
              <div className="grid grid-cols-2 gap-4">
                <select
                  ...
                  className="border border-[#C2A56B] rounded-xl p-3"
                >
              ...
              <div>
                <div className="flex items-center justify-between mb-4">
                  <h3 className="font-semibold text-[#1F4D46]">Produtos utilizados</h3>
                  <button
                    type="button"
                    onClick={addProduct}
                    className="bg-[#1F4D46] text-white px-3 py-2 rounded-lg text-sm"
                  >
                    + Produto
                  </button>
                </div>
                  ...
                  {selectedProducts.map((item, index) => (
                    <div key={index} className="grid grid-cols-12 gap-3">
                      ...
                      <input
                        ...
                        placeholder="Qtd"
                        className="col-span-2 border border-[#C2A56B] rounded-xl p-3"
                      />
              ...
              <div className="flex justify-end gap-3 pt-4">
                <button
                  ...
                  className="border border-[#C2A56B] px-4 py-2 rounded-xl"
                >
                  Cancelar
                </button>
                <button
                  onClick={saveProcedure}
                  className="bg-[#1F4D46] hover:bg-[#285A50] text-white px-5 py-2 rounded-xl"
                >
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-60 p-6">
          <div className="bg-white rounded-2xl w-full max-w-md p-6">
            <h2 className="text-2xl font-bold text-[#1F4D46] mb-3">Excluir procedimento</h2>
