    ...
    return (
      <div className="bg-creme-50 border border-creme-200 rounded-2xl p-6">
        <div className="flex items-center justify-between mb-1">
          <h2 className="text-xl font-bold text-verde">Preencher Anamnese</h2>
              ...
              <div className="w-56 shrink-0">
                {q.type === "text" && (
                  <input
                    ...
                    className="w-full border border-creme-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-verde/20"
                  />
                )}
                {q.type === "boolean" && (
                  <select
  ...
  return (
    <div className="bg-creme-50 border border-creme-200 rounded-2xl p-6">
      <div className="flex items-center justify-between mb-5">
        <h2 className="text-xl font-bold text-verde">Anamneses</h2>
            ...
            <div key={r.id} className="bg-white rounded-xl px-4 py-3 flex items-center justify-between">
              ...
              {r.status === "finalized" ? (
                ...
                <button onClick={() => startFilling(r.template ? { id: r.templateId, name: r.template.name, questions: r.template.questions } : null, r)}
                  className="text-xs text-verde font-medium hover:underline">
                  Continuar
                </button>
              )}
            </div>
