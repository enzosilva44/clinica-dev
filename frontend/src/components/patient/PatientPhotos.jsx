    ...
    <div className="bg-creme-50 border border-creme-200 rounded-2xl overflow-hidden">
      ...
      <div
        ...
        className={`border-2 border-dashed rounded-xl mx-5 mt-5 p-8 text-center cursor-pointer transition-all ${
          dragging
            ? "border-verde bg-creme-100"
            : "border-ambar hover:border-verde/40 hover:bg-white"
        }`}
      >
        <input
          ref={fileRef}
        ...
        {uploading ? (
          <div className="flex flex-col items-center gap-2">
            <div className="w-8 h-8 border-2 border-verde/30 border-t-verde rounded-full animate-spin" />
            <p className="text-sm text-gray-500">Enviando fotos…</p>
          </div>
        ) : (
          <div className="flex flex-col items-center gap-2">
            <div className="w-10 h-10 rounded-xl bg-creme-100 flex items-center justify-center">
              <Upload size={18} className="text-verde" />
            </div>
      ...
      <div className="p-5">
        {loading ? (
          <div className="space-y-3">
            {[1, 2].map((i) => <div key={i} className="h-32 bg-creme-100 rounded-xl animate-pulse" />)}
          </div>
        ) : photos.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-14 gap-3">
            <div className="w-14 h-14 rounded-2xl bg-creme-100 flex items-center justify-center">
              <ImageOff size={22} className="text-ambar" />
            </div>
            ...
            {groups.map(([date, items]) => (
              <div key={date}>
                ...
                <div className="flex items-center gap-3 mb-3">
                  <span className="text-xs font-bold text-verde uppercase tracking-wide whitespace-nowrap">
                    {date}
                  </span>
                  <div className="flex-1 h-px bg-creme-200" />
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                  {items.map((photo) => (
                    <div key={photo.id} className="group relative aspect-square rounded-xl overflow-hidden bg-creme-100">
                      <img
                      ...
                      <div className="absolute inset-0 bg-black/0 group-hover:bg-black/35 transition-all duration-200 flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100">
                        ...
                        >
                          <ZoomIn size={14} className="text-verde" />
                        </button>
                        <button
